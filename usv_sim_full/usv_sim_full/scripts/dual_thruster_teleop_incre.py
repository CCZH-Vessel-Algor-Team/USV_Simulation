...
class IncrementalTeleop(Node):
    def __init__(self):
        ...
        self.pub_l_thrust = self.create_publisher(Float64, left_thrust_topic, 10)
        self.pub_l_pos = self.create_publisher(Float64, left_pos_topic, 10)
        self.pub_r_thrust = self.create_publisher(Float64, right_thrust_topic, 10)
        self.pub_r_pos = self.create_publisher(Float64, right_pos_topic, 10)

        self.get_logger().info(
            f'增量遥控已启动 (namespace={self.namespace}): '
            f'{left_thrust_topic}, {left_pos_topic}, '
            f'{right_thrust_topic}, {right_pos_topic}'
        )
...
def main(args=None):
    ...
    try:
        ...
        msg_zero = Float64()
        msg_zero.data = 0.0
        node.pub_l_thrust.publish(msg_zero)
        node.pub_l_pos.publish(msg_zero)
        node.pub_r_thrust.publish(msg_zero)
        node.pub_r_pos.publish(msg_zero)
